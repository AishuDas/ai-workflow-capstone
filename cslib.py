...
def fetch_data(data_dir):
    ...
    master_df = pd.concat(list(all_months.values()), sort=True)
    years, months, days = master_df['year'].values, master_df['month'].values, master_df['day'].values
    dates = ["{}-{}-{}".format(years[i], str(months[i]).zfill(2), str(days[i]).zfill(2)) for i in range(master_df.shape[0])]
    master_df['invoice_date'] = np.array(dates, dtype='datetime64[D]')
    master_df['invoice'] = [re.sub("\D+", "", i) for i in master_df['invoice'].values]

    ## sort by date and reset the index
    master_df.sort_values(by='invoice_date', inplace=True)
...
def convert_to_ts(df_orig, country=None):
    ...
    if country:
        ...
        orig_df = df_orig[mask]
    else:
        orig_df = df_orig

    ## use a date range to ensure all days are accounted for in the data
    invoice_dates = orig_df['invoice_date'].values
    start_month = '{}-{}'.format(orig_df['year'].values[0], str(orig_df['month'].values[0]).zfill(2))
    stop_month = '{}-{}'.format(orig_df['year'].values[-1], str(orig_df['month'].values[-1]).zfill(2))
    df_dates = orig_df['invoice_date'].values.astype('datetime64[D]')
    days = np.arange(start_month, stop_month, dtype='datetime64[D]')

    purchases = np.array([np.where(df_dates == day)[0].size for day in days])
    invoices = [np.unique(orig_df[df_dates == day]['invoice'].values).size for day in days]
    streams = [np.unique(orig_df[df_dates == day]['stream_id'].values).size for day in days]
    views = [orig_df[df_dates == day]['times_viewed'].values.sum() for day in days]
    revenue = [orig_df[df_dates == day]['price'].values.sum() for day in days]
